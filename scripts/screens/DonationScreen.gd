...
func _ready() -> void:
	...
	var sub := _label("Eight requests have arrived at your office. You have the means to fund one.", 22, MUTED)
	...
	var info := _label("Cash on hand: $%s    Donation cost: $%s" % [
		...
		_fmt(float(BalanceConfig.game_params().get("donation_cost", 2_500_000_000)))], 22, GOLD)
	...
	skip.custom_minimum_size = Vector2(500, 48)
...
func _build_cause_card(cause: Dictionary) -> Control:
	...
	v.add_child(_label(String(cause.name), 22, TEXT))
	v.add_child(_label("— %s" % String(cause.character), 16, MUTED))

	var plea := _label("\"%s\"" % String(cause.plea), 22, TEXT)
	plea.add_theme_constant_override("line_separation", 6)
